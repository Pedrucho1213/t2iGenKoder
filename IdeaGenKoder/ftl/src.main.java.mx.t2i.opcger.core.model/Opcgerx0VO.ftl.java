...
<#list FTL_searchAttributes as FTL_searchAttribute>
            if ( entry.getKey().equals( "${FTL_CORE_PROGRAM}.${FTL_searchAttribute.capitalizedCoreName}" ) ) { set${FTL_searchAttribute.javaNameCapitalizingFirstLetter}( ${FTL_searchAttribute.parsedValueFromEntry} ); }
</#list>
<#list FTL_fieldAttributes as FTL_fieldAttribute>
            if ( entry.getKey().equals( "${FTL_CORE_PROGRAM}.${FTL_fieldAttribute.capitalizedCoreName}" ) ) { set${FTL_fieldAttribute.javaNameCapitalizingFirstLetter}( ${FTL_fieldAttribute.parsedValueFromEntry} ); }
</#list>
...
<#-- Comentar para omitir las declaraciones en duro: ----------------
            ...
            if ( entry.getKey().equals( "OPCGERX0.FINIVIG" ) ) { setFinivig( Integer.parseInt(entry.getValue()) ); }
...
/ * 2021-03-05 ati: No para objetos incrustados */
     ...
     *
     */
    public static List< ${FTL_Model} > auxGetListFromCore( Map< String, String > mappingFields )
    {
...
<#list FTL_searchAttributes as FTL_searchAttribute>
                if ( entry.getKey().endsWith( ".${FTL_searchAttribute.capitalizedCoreName}" ) ) { item.set${FTL_searchAttribute.javaNameCapitalizingFirstLetter}( ${FTL_searchAttribute.parsedValueFromEntry} ); }
</#list>
<#list FTL_fieldAttributes as FTL_fieldAttribute>
                if ( entry.getKey().endsWith( ".${FTL_fieldAttribute.capitalizedCoreName}" ) ) { item.set${FTL_fieldAttribute.javaNameCapitalizingFirstLetter}( ${FTL_fieldAttribute.parsedValueFromEntry} ); }
</#list>
