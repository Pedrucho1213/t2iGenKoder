...
@Service
public class RegistrarSolicitudServiceService {
	/** The logger. */
	private final Logger log = LoggerFactory.getLogger(RegistrarSolicitudServiceService.class);

	@Autowired
	private Catclix0CoreService Catclix0CoreService;

	@Autowired
	private RespRegistrarSolicitudService wsModel;
	 ...
	 */
	public RespRegistrarSolicitudService registrarSolicitud(ReqRegistrarSolicitudService reqRegistrarSolicitudService)
			throws ServiceException {
		RespRegistrarSolicitudService respuesta = null;

		String contrat = reqRegistrarSolicitudService.getCliente().getContrat();

		if (contrat.contentEquals("DUMMY")) {
			respuesta = registrarSolicitudDummy( reqRegistrarSolicitudService );
		} else if (contrat.contentEquals("ERROR")) {
			respuesta = registrarSolicitudDummyError( reqRegistrarSolicitudService );
		} else if (contrat.contentEquals("EXCEPTION")) {
			respuesta = registrarSolicitudDummyException( reqRegistrarSolicitudService );
		} else {
			respuesta = registrarSolicitudDesdeCore( reqRegistrarSolicitudService );
		}
		return respuesta;
	}

	public RespRegistrarSolicitudService registrarSolicitudDesdeCore(ReqRegistrarSolicitudService reqRegistrarSolicitudService)
			throws ServiceException {
		RespRegistrarSolicitudService respuesta = new RespRegistrarSolicitudService();
...
///			throw ServiceExceptionDetails.buildServiceException("-1", msg, new String[] { msg });
	...
	protected void catclix0CoreModelToWsModel(catclix0 coreModel, RespRegistrarSolicitudService wsModel)
			throws ServiceException {
		wsModel.setMessage(coreModel.getMessage());
	}

	protected void catclix0CoreModelWsModelToCoreModel(ReqRegistrarSolicitudService wsModel, catclix0 coreModel)
			throws ServiceException {
	...
	public RespRegistrarSolicitudService registrarSolicitudServiceDummy(ReqRegistrarSolicitudService reqRegistrarSolicitudService) throws ServiceException
	{
//		RespRegistrarSolicitud wsModel = new RespRegistrarSolicitud();
	...
	public RespRegistrarSolicitudService registrarSolicitudServiceDummyError(ReqRegistrarSolicitudService reqRegistrarSolicitudService)
			throws ServiceException {
		RespRegistrarSolicitudService respRegistrarSolicitudService = new RespRegistrarSolicitudService();
		...
		if (contrat.contentEquals("ERROR")) {
			...
			log.warn(msg);
			respRegistrarSolicitudService.setStatusCode(-1);
			respRegistrarSolicitudService.setMessage(msg);
		}
		return respRegistrarSolicitudService;
	}

	public RespRegistrarSolicitudService registrarSolicitudServiceDummyException(ReqRegistrarSolicitudService reqRegistrarSolicitud)
			throws ServiceException {
		RespRegistrarSolicitudService respRegistrarSolicitudService = new RespRegistrarSolicitudService();
